...
public class ExplosionMixin {
	...
	public BlockPos wrapBlockPos(double x, double y, double z) {
		return level.getTransformer().SSO().Block.wrap(BlockPos.containing(x, y, z));
	}
	...
	public double unwrapEntityX(Entity instance) {
		return level.getTransformer().SSO().Coord.X.unwrap(instance.getX(), x);
	}
	...
	public double unwrapEntityZ(Entity instance) {
		return level.getTransformer().SSO().Coord.Z.unwrap(instance.getZ(), z);
	}
}
